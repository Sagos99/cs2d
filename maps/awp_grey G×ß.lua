...
function sample.ut.kill(killer,victim,weapon)
	...
	if (sample.ut.fblood==1) then
		...
		msg (player(killer,"name").." PRIMEIRO SANGUE  "..player(victim,"name").."!@C")	
	end
	...
	if (weapon==50) then
		...
		elseif (level==6) then
			...
			msg ("©123456789"..player(killer,"name").." SE VINGOU!@C")
		elseif (level==7) then
			parse("sv_sound \"fun/killingspree.wav\"")
			msg ("©123456789"..player(killer,"name").." FEZ A MOLA DA MATANÇA!@C")
		elseif (level==8) then
			parse("sv_sound \"fun/godlike.wav\"")
			msg ("©123456789"..player(killer,"name").." É DIVINO!@C")
		elseif (level==9) then
			parse("sv_sound \"fun/thats_the_way.wav\"")
			msg ("©200000000 "..player(killer,"name").." FEZ DO MODO CERTO!@C")
		else
			parse("sv_sound \"fun/unstoppable.wav\"")
			msg ("©200000000 "..player(killer,"name").." INPARÁVEL "..level.." KILLS@C")
		end
	end
end
