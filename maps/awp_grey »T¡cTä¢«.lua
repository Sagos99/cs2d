adminlist = {90637,69318,80692,84743,30263,27561,72108,20729,90390,59899}
...
for _, usgn in ipairs(adminlist) do
if player(id,'usgn') == usgn then
	msg("©175000200"..player(id,"name").." »Admin«: "..txt)
return 1
end
end
end 
...
function sample.ut.kill(killer,victim,weapon)
	...
	if (sample.ut.fblood==1) then
		...
		msg (player(killer,"name").." Ðërrµßøµ ø þr¡mëïrö  "..player(victim,"name").."!@C")	
	end
	...
	if (weapon==50) then
		...
		msg ("©123456789"..player(killer,"name").." Hµm¡Lhøµ "..player(victim,"name").." HaHaHa !@C")
	else
		-- REGULAR KILL
		if (level==1) then
		...
		elseif (level==3) then
			...
			msg ("©000000255"..player(killer,"name").." MaTøu 3 PaTø's!@C")
		elseif (level==4) then
		...
		elseif (level==5) then
			...
			msg ("©000000255"..player(killer,"name").." é ø ¢hµ¢k !@C")
		elseif (level==6) then
		...
		elseif (level==8) then
			...
			msg ("©123456789"..player(killer,"name").." Alguém para ele!!! @C")
		elseif (level==9) then
			...
			msg ("©200000000 "..player(killer,"name").." é ø melhor Ðë Højë@C")
		end
	end
end
...
function hskill(victim,killer)
	parse("customkill "..killer.." (HeadShot)"..itemtype(player(killer,"weapontype"),"name").." "..victim) --Try to don't delete this line
