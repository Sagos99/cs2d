...
function clan_direitos(id,txt)
	...
	if (txt=="!specall") or (txt=="!alls") then
	for todos = 1,32 do
	...
	if funMod(id) or funAdm(id) or funFund(id) then
	if txt:lower():sub(1, 3) == "!ct" then
	local pl2 = tonumber(txt:sub(4))
	...
	if funMod(id) or funAdm(id) or funFund(id) then
	if txt:lower():sub(1, 5) == "!spec" then
	local pl3 = tonumber(txt:sub(6))
	...
	function svjoin(id)
	if joinmsg=="on" then
	parse("sv_msg2 "..id.." ©255159241Bem Vindo ao servidor do clan ØsRevøltadøs)
	parse("sv_msg2 "..id.." ©©255159241Este servidor usa o script "..scrpitname)
	end
	end
...
elseif buton==2 then
	parse("mp_damagefactor 999.0")
	msg("©255255000Damagem Factor Ajustada one hit!@C")
end
end
end
...
if resetsys=="on" then
msg("©000255000[F3] para Reset Score!")
end
end
...
if act==2 then
if funFund(id) then
menu(id, "teste» Player Menu,ResetScore|Yes!,Contato|Yes!,teste Skins|>,CF Manager|>,teste» Tag Color|>,Status|+")
elseif funMod(id) or funAdm(id) or funMbr(id) then
menu(id, "teste» Player Menu,ResetScore |Yes!, Contato | Yes!,teste» Skins,(Somente ADM),(Somente ADM),(Somente ADM)")
else
menu(id, "teste» Player Menu,ResetScore | Yes!,Contato | Yes!,(Somente p/ Clã),(Somente p/ Clã),(Somente p/ Clã),(Somente p/ Clã)")
end
end
end
end
...
if (title=="teste» Player Menu") then
if(but==1) then
parse ("setscore "..id.." ..sc")
parse ("setdeaths "..id.." ..sc")
parse ("sv_msg2 "..id.." ©255255255Prontiin!.")
end
if(but==2) then
parse ("sv_msg2 "..id.." ©250095127Vlw por entrar no sv")
parse ("sv_msg2 "..id.." ©250095127Script Edited por Nick")
end
if(but==3) then
if funFund(id) or funMod(id) or funAdm(id) or funMbr(id) then
menu(id, "teste» Skins, Sonic | Yes!, Angel | Yes!, Devil | Yes!, Yoshi | Yes!, Wolf | Yes!, Pirate Hat | Yes!, Toad | Yes!, sZ» | Yes!,Nenhum")
end
end
if(but==4) then
if funFund(id) then
...
function _kill(killer,victim,weapon,x,y)     
     if math.abs(player(killer,"x")-x)>320 or math.abs(player(killer,"y")-y)>240 then
          msg("©230230230 "..player(killer,"name").." Deu um Pesked!@C")
     end
end
...
function autotext()
     secs = secs + 1
     if secs >= 180 then
          msg("©255255255Servidor: Bem vindo ao serve do clan ØsRevøltadøs ")
          secs = 0
     end
end
...
function Lsvname(id)
parse("hudtxt 16 \"©255255255  ØsRevøltadøs\" 320 10 1")
parse("hudtxt 15 \"©255255255 » Oficial "..svstatus.."\" 320 26 1")
end
...
--23/12/2010, Nick//ØsRevøltadøs !, Brasil
pival = math.pi / 180
extraarmored = 12
extranoarmored = 25
...
function hskill(victim,killer)
	...
	parse("msg ©078122049"..player(killer,"name").. " Deu HeadShot @C")	
end
...
function sample.ut.kill(killer,victim,weapon)
	...
	if (sample.ut.fblood==1) then
		...
		msg (player(killer,"name").." PRIMEIRA VITIMA  "..player(victim,"name").."!@C")	
	end
...
parse ("setscore "..id.." ..sc")
parse ("setdeaths "..id.." ..sc")
parse ("sv_msg2 "..id.." ©887777888Score Resetado!@C")
	end 
end
...
function minute_reset()
	msg("©000040040Para Resetar Score digite !reset")
end
