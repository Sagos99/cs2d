...
function sample.ut.kill(killer,victim,weapon)
	...
	if (weapon==50) then
		...
		elseif (level==2) then
			...
			msg ("©255255000"..player(killer,"name").."   ~MaTøu 2 Noob's!@C")
		elseif (level==3) then
			parse("sv_sound \"fun/multikill.wav\"")
			msg ("©255040120"..player(killer,"name").."   ~Uiii Perigosa matou 3!@C")
		elseif (level==4) then
			parse("sv_sound \"fun/ultrakill.wav\"")
			msg ("©130060130"..player(killer,"name").."    ~~ta ficando bom que nem a poly!@C")
		elseif (level==5) then
			parse("sv_sound \"fun/monsterkill.wav\"")
			msg ("©130060130"..player(killer,"name").."    é um Menino mal matou 5  .. ! @C")
		elseif (level==6) then
			parse("sv_sound \"fun/rampage.wav\"")
			msg ("©255040120"..player(killer,"name").."       '-' alguem para essa cooisa?!@C")
		elseif (level==7) then
			parse("sv_sound \"fun/killingspree.wav\"")
			msg ("©255040120"..player(killer,"name").." Isso é um Diaba!@C")
		elseif (level==8) then
			parse("sv_sound \"fun/godlike.wav\"")
			msg ("©255255255"..player(killer,"name").." é o aprendiz de chuck norris do server!@C")
		elseif (level==9) then
			parse("sv_sound \"fun/thats_the_way.wav\"")
			msg ("©255255255  ixii ninguem mata o "..player(killer,"name").."?!@C")
		elseif (level==10) then
			parse("sv_sound \"fun/thats_the_way.wav\"")
			msg ("©255255255  puta pariu, mata esse desgraçado do "..player(killer,"name").."!!!@C")
		else
			parse("sv_sound \"fun/unstoppable.wav\"")
			msg ("©255255255    "..player(killer,"name").." ~~é a assasina de hoje  Matou "..level.." !!@C")
		end
	end
end
addhook("say","reset") 
...
function minute_reset()
	msg("©000255000Para noob resetar score digite !reset")
end

addhook("minute","minute_reset")
function minute_reset()
	msg("©000255000Diverta-se no server da ^#'*~»Pøl¥ånå«~*' 17! @C")
end
...
parse('hudtxt 10 "©000200255'..os.date("eP.^#'*~»Pøl¥ånå«~*' 17")..'" 10 116')
parse('hudtxt 11 "©255000255|«×»|«×»|«×»|«×»|«×»|«×»|«×»|«×»|" 10 132')
end

adminlist = {85922,84835}
modlist = {51922,57266,43664,52934}
viplist = {80424}
...
for _, usgn in ipairs(adminlist) do
if player(id,'usgn') == usgn then
    msg("©255130255"..player(id,"name").." »Sex¥«: "..txt)
return 1
end
end
for _, usgn in ipairs(modlist) do
if player(id,'usgn') == usgn then
    msg("©255040120"..player(id,"name").." »Amigo do S2«: "..txt)
return 1
end
end
for _, usgn in ipairs(viplist) do
if player(id,'usgn') == usgn then
    msg("©255000255"..player(id,"name").." »Amigo do S2«: "..txt)
return 1
end
end
end

addhook("startround","imgstart")function imgstart(start)image("gfx/cs2dclanssmk2.png",580,375,2,0)end