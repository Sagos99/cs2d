...
function sample.ut.kill(killer,victim,weapon)
	...
	if (sample.ut.fblood==0) then
		...
		msg (""..player(killer,"name").." Levooou primeiro o "..player(victim,"name").." ! ")
	end
	-- HUMILIATION? (KNIFEKILL)
	if (weapon==50) then
		-- HUMILIATION!
		parse("sv_sound \"fun/knife.wav\""); 
		msg ("©201052255"..player(killer,"name").." Toomou facada troxa "..player(victim,"name").." ! ")
	else
		-- REGULAR KILL
		if (level==1) then
			-- Single Kill! Nothing Special!
		elseif (level==2) then
			parse("sv_sound \"fun/doublekill2.wav\"");
			msg (""..player(killer,"name").." matou 2 Noob's !")
		elseif (level==3) then
			parse("sv_sound \"fun/multikill.wav\"")
			msg (""..player(killer,"name").." Derrubou 3 Bobões !")
		elseif (level==4) then
			parse("sv_sound \"fun/monsterkill2.wav\"")
			msg (""..player(killer,"name").." Levoou 4 pro Inferno !")
		elseif (level==5) then
			parse("sv_sound \"fun/holyshit.wav\"")
			msg (""..player(killer,"name").." Uuuiiii bixa má !!")
		else
			parse("sv_sound \"fun/Picoso.wav\"")
			msg (""..player(killer,"name").." Ta igual o NanDiiÑ")
		end
	end
end
...
parse ("sv_msg2 "..id.." ©255358999Prontiin ! ;D")
	end 
end

addhook("minute","minute_reset")
function minute_reset()
	msg("©000255000Frag Feio ? !rs")
end
...
function hskill(victim,killer)
	...
	Thanks to Crawis for "headshot2.wav" and "headshot2.wav"
	]]
	...
	parse("msg ©255240354"..player(killer,"name").. " AH HAAA. Toomou um heead @C")	
end
...
for _, usgn in ipairs(adminlist) do
if player(id,'usgn') == usgn then
    msg("©255130255"..player(id,"name").."  »Admiin. : "..txt)
return 1
end
end
...
if player(id,'usgn') == usgn then
    msg("©255130255"..player(id,"name").." »Vip«: "..txt)
return 1
end
end

for _, usgn in ipairs(girlist) do
if player(id,'usgn') == usgn then
    msg("©210009198"..player(id,"name").." »Sexy«: "..txt)
return 1
end
end
...
if player(id,'usgn') == usgn then
    msg("©250250250"..player(id,"name").." »MëmB®øS«: "..txt)
return 1
end
end
end
