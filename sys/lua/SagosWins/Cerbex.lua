...
ativado = 0
addhook("serveraction","indigoway")
function indigoway(id,act)
    if act == 1 and player(id,"usgn") == ADM then
        if ativado == 0 then
            ativado = 1
            parse("sv_msg2 "..id.." ©000255000Ativado")
        elseif ativado == 1 then
            ativado = 0
            parse("sv_msg2 "..id.." ©255050050Desativado")
        end
    end
end

addhook("say","falandoprobot")
function falandoprobot(id,txt)
    if ativado == 1 and player(id,"usgn") == ADM then
        fala = txt
        return 1
    end
end
